...
def find_pairs(N, M):
    """Находит все пары чисел от 1 до N, сумма которых не превышает M"""
    pairs = []
    
    # Внешний цикл для первого числа
    for i in range(1, N + 1):
        # Внутренний цикл для второго числа
        for j in range(i, N + 1):  # Начинаем с i, чтобы избежать дубликатов
            if i + j <= M:
                pairs.append((i, j))
            else:
                break  # Если сумма превысила M, переходим к следующему i
